...
private:
	...
	public:
		Info() {
			isPaused = repeat = isGood = false;
		}
		bool isPaused, repeat, isGood;
		string fileName;
	};
...
public:
	...
	list<CMovingBitmap>::iterator	GetCurrentBitmapIter();
	void  SetCurrentBitmapIter(list<CMovingBitmap>::iterator);
	int   Height();					// 取得動畫的高度
